...
describe('Component: WelcomeScreen', () => {
  ...
  it('should redirect to /game when user clicked to start button', () => {
    ...
    const useDispatch = jest.spyOn(Redux, 'useDispatch'); // Позволяет шпионить в переданной сущности пр. Redux за диспатчем. А потом можно сделать тесты (см. далее). По сути это возможность использовать jest.fn но уже на существующей функции
    ...
    expect(useDispatch).toBeCalledTimes(1); // вызывался ли use Dispatch?
    expect(dispatch).nthCalledWith(1, {
      type: `${NameSpace.game}/resetGame`,
    }); // или dipatch вызывался с указанной полезной нагрузкой?
  });
});
